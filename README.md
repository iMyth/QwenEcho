...
├── ios/sherpa_onnx_local/          # Local SPM package for SherpaOnnx
│   ├── Package.swift               # Declares the `SherpaOnnx` library product
│   ├── prepare.sh                  # Downloads + merges upstream XCFrameworks
│   ├── combine-libs.sh             # Merges onnxruntime into sherpa-onnx slices
│   ├── Sources/SherpaOnnx/         # Swift API wrapper (~2250 lines over C API)
│   └── sherpa-onnx.xcframework/    # Combined binary (generated by prepare.sh)
...
#### iOS native dependencies (one-time setup)

ASR uses a local Swift Package Manager package that wraps a combined XCFramework
(sherpa-onnx + onnxruntime merged into a single binary to sidestep an Xcode
duplicate-module bug). On a fresh clone, generate it once:

```bash
bash ios/sherpa_onnx_local/prepare.sh
```

This downloads the upstream XCFrameworks and invokes `combine-libs.sh` to merge
them. The resulting `sherpa-onnx.xcframework/` is gitignored (~120 MB).
